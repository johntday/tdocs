...
getOneAway = function(graph) {
	...
	var parent;
	var nouns = {
		noun: createJoinRect(noun.class_name, noun.text.trunc(20), null, 12, 'blue', null, null, null)
	};
	if (noun.parent && noun.parent !== '#') {
		parent = Nouns.findOne(noun.parent, {field: {title: 1}});
		nouns.parent = createJoinRect(noun.class_name, parent.title.trunc(20), null, 10, null, '/nouns/'+noun.parent, null, null);
	}
	if (noun.children && noun.children.length>0) {
		for (var i=0; i < noun.children.length; i++) {
			...
			nouns[ noun.children[i] ] = createJoinRect(noun.class_name, child.title.trunc(20), null, 10, null, '/nouns/'+noun.children[i], null, null);
		}
	}
	...
	if (rels && rels.length > 0) {
		_.each(rels, function(rel){
			rel_nouns.push( createJoinRect(noun.class_name, rel.target_title.trunc(20), null, 10, null, '/nouns/'+rel.target_id, null, null) );
		});
	}
	...
	if (rels && rels.length > 0) {
		_.each(rels, function(rel){
			rel_nouns.push( createJoinRect(noun.class_name, rel.source_title.trunc(20), null, 10, null, '/nouns/'+rel.source_id, null, null) );
		});
	}
	_.each(rel_nouns, function(c) { graph.addCell(c); });
	//
	relations = [];
	_.each(rel_nouns, function(c, idx) {
		relations.push( createRelLink( rels[idx].rel_name, c.id, nouns.noun.id ) );
	});
	_.each(relations, function(r) { graph.addCell(r); });

};

createJoinRect = function(class_name, text, textColor, textFontSize, fillColor, link, strokeColor, strokeWidth) {
	var attrs = {
		rect: { rx: 2, ry: 2, width: 50, height: 30, fill: fillColor||'#27AE60'/*, stroke: strokeColor||'#D35400', 'stroke-width': strokeWidth||5*/ },
		text: { text: text, fill: textColor||'white', 'font-size': textFontSize||10, class_name: class_name }
	};
...
createRelLink = function(rel_name, source_id, target_id) {
	...
	if (rel_name === 'assigns')
		return new joint.dia.Link({ source: { id: source_id }, target: { id: target_id }});
	if (rel_name === 'trigger')
		return new joint.shapes.uml.Trigger({ source: { id: source_id }, target: { id: target_id }});
