...
graphCurrentRelationships = function() {
	...
	if (element_ids) {
		var rels = Relationships.find({project_id:getProjectId(), source_id: {$in:element_ids}}).fetch();
		...
		//////////////////////////
		rels = Relationships.find({project_id:getProjectId(), target_id: {$in:element_ids}}).fetch();
