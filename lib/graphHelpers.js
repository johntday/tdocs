getOneAway = function(graph) {
	...
	if (noun.children && noun.children.length>0) {
		for (var i=0; i < noun.children.length; i++) {
			...
			nouns[ noun.children[i] ] = createNounFactory(noun.id, noun.class_name, child.title.trunc(20), null, 10, null, '/nouns/'+noun.children[i], null, null, child.type);
		}
	}
...
addNounToGraph = function(noun) {
	...
	if (doAdd) {
		graph.addCell( createNounFactory(noun._id, noun.class_name, noun.title.trunc(20), null, 10, null, null, null, null, noun.type) );
...
function getNounColors(class_name) {
	...
	switch( area_code ){
		// [FILL, TEXT, STROKE]
		case 'a': colors = ['#A9E2F3'/*blue*/, '#000000', '#2E2EFE']; break;
