...
getOneAway = function(graph) {
	...
	var parent;
	var nouns = {
		noun: createJoinRect(noun.id, noun.class_name, noun.text.trunc(20), null, 12, 'blue', null, null, null)
	};
	if (noun.parent && noun.parent !== '#') {
		parent = Nouns.findOne(noun.parent, {field: {title: 1}});
		nouns.parent = createJoinRect(noun.id, noun.class_name, parent.title.trunc(20), null, 10, null, '/nouns/'+noun.parent, null, null);
	}
	...
	if (rels && rels.length > 0) {
		_.each(rels, function(rel){
			rel_nouns.push( createJoinRect(noun.id, noun.class_name, rel.target_title.trunc(20), null, 10, null, '/nouns/'+rel.target_id, null, null) );
		});
	}
	...
	if (rels && rels.length > 0) {
		_.each(rels, function(rel){
			rel_nouns.push( createJoinRect(noun.id, noun.class_name, rel.source_title.trunc(20), null, 10, null, '/nouns/'+rel.source_id, null, null) );
		});
	}
	_.each(rel_nouns, function(c) { graph.addCell(c); });
	//
...
createJoinRect = function(_id, class_name, text, textColor, textFontSize, fillColor, link, strokeColor, strokeWidth) {
	var attrs = {
		...
		text: { text: text, fill: textColor||'white', 'font-size': textFontSize||10, class_name: class_name, _id: _id }
	};
...
createRelLink = function(_id, rel_name, source_id, target_id) {
...
addRelToGraph = function(_id, rel_name, source_graph_id, target_graph_id) {
	Template['tmpl_graphDgm_detail'].graph.addCell( createRelLink(_id, rel_name, source_graph_id, target_graph_id) );
};