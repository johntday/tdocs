...
validateGraph = function(graph) {
	var hasInputError = validateFieldLength(graph.title, 'Title', 3, 128);
	return hasInputError;
};
// Transform graph before save.
transformGraph = function(graph) {
	return graph;
};
 ...
 * VALIDATE Project
 */
validateProject = function(project, _id) {
	var hasInputError = validateFieldLength(project.title, 'Title', 3, 128)
	//|| validateFieldLength(diagram.description, 'Description', 10, 2048)
	return hasInputError;
};
// Transform project before save.
transformProject = function(project) {
	return project;
};
