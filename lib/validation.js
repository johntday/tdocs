...
validateTdoc = function(tdoc) {
	var hasInputError = isLoggedIn()
		|| validateFieldLength(tdoc.title, 'Title', 10, 128)
...
validateDiagram = function(diagram) {
	var hasInputError = validateFieldLength(diagram.title, 'Title', 10, 128)
 ...
 * VALIDATE Glossary
 */
validateGlossary = function(glossary) {
	var hasInputError = validateFieldLength(glossary.title, 'Title', 3, 128)
