if (Meteor.isClient) {
	...
	getProjectId = function() {
		return Session.get('project')._id;
	};
	getProjectTitle = function() {
		return Session.get('project').title;
	};
	getProjectOwnerId = function() {
		return Session.get('project').userId;
	};
	setProject = function(project) {
		if (project)
			Session.set('project', {_id: project._id, title: project.title, userId: project.userId});
		else
			Session.set('project', {_id: null, title: null, userId: null});
	};
    ...
    Meteor.MyClientModule = (function () {
		 ...
		 */
	    oPublic.appConfig = {
		    pageLimit: 20, pageLimitMid: 50, pageLimitMax: 1000,
