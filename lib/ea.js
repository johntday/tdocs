ea = {
	...
	getPossibleEARelsForRelationship: function(class_name) {
		...
		var ea_rels = EA_Relationships.find({}).fetch();
		var class_names = {};
		_.each(ea_rels, function(ea_rel){
			if (ea_rel.source===class_name) {
				class_names[class_name + ea_rel.target] = {semantic:ea_rel.semantic, rel_name:ea_rel.rel_name};
			} else if (ea_rel.target===class_name) {
				class_names[class_name + ea_rel.source] = {semantic:ea_rel.r_semantic, rel_name:ea_rel.rel_name};
			}
		});
		return class_names;
	},
