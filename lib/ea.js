ea = {class_name:{}, modelArea:{}, modelLayer:{}, classBelongsToArea:{},
	...
	getRelationship: function(source_class_name, target_class_name) {
		...
		if (rel) { return rel; }
		return _.findWhere(ea.relationships, {source: target_class_name, target: source_class_name} );
	},
	buildRelationshipText: function(source_class_name, source_title, target_class_name, target_title) {
		var rel = ea.getRelationship(source_class_name, target_class_name);
		if (!rel) { return 'UNKNOWN'; }
		return '"' + source_title + '" ' + rel.semantic + ' "' + target_title + '"';
	},
	...
	/**
	 * if options.class_name, then look both directions in source and target.
	 * if options.both=true, look both ways with source and target.
	 * @param options
	 * @returns {Array}
	 */
	getRelationships: function(options) {
		if (!options) { return []; }
		var list;
//		if (options.class_name) {
//			return _.filter(ea.relationships, function(rel){
//				return (rel.source===options.class_name || rel.target===options.class_name);
//			});
//		}
//		return _.where(ea.relationships, options);
		if (options.both) {
			list = _.filter(ea.relationships, function(rel){
				return (rel.source===options.source && rel.target===options.target)
					|| (rel.source===options.target && rel.target===options.source);
			});
		} else {
			list = _.where(ea.relationships, options);
		}
		return list;
	},
	...
	getRelationshipSemantic: function(source_class_name, target_class_name) {
		if (!source_class_name || !target_class_name) { return 'UNKNOWN'; }
