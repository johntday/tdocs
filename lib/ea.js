ea = {
	...
	getRelationshipSemantic: function(source_class_name, target_class_name, rel_name) {
		var rel = ea.relationships_index[source_class_name+target_class_name+rel_name];
		if (rel){ return rel.semantic; }
...
//			return {semantic: rel.r_semantic, rel_name: rel.rel_name};
	...
	hasRelationship: function(source_class_name, target_class_name, both) {
		if (_.findWhere(ea.relationships, {source:source_class_name, target:target_class_name})) { return true; }
		if (both && _.findWhere(ea.relationships, {source:target_class_name, target:source_class_name})) { return true; }
		return false;
	},
	getRelationships: function(source_class_name, target_class_name, both) {
		var results = {}, reverse;
		var forward = _.where(ea.relationships, {source:source_class_name, target:target_class_name});
		if (both)
			reverse = _.where(ea.relationships, {source:target_class_name, target:source_class_name});
