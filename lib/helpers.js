...
gotoNounFilterPage = function(key, filter) {
	NounsFilter.filter.clear();
	NounsFilter.filter.set(key, filter);
	Router.go('/nouns');
};
searchRouteLogic = function() {
//	if ( _.contains(['/sciFiMovies','/timelines','/persons','/favs','/stars'], Router.current().path) ) {
...
generateSlug = function(text, project_id) {
	if (!text) return '';
	// 1) convert to lowercase
	// 2) remove dashes and pluses
	// 3) replace spaces with dashes
	// 4) remove everything but alphanumeric characters and dashes
	return text.toLowerCase().replace(/-+/g, '').replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') + project_id;
};
getNow = function() {
